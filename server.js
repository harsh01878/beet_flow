...
const MONGO_URI = process.env.MONGO_URI ||
  'mongodb+srv://harshjha589_db_user:gJquwRIIo26kn4YY@cluster0.zodzwqv.mongodb.net/sonichub?appName=Cluster0';
...
const songSchema = new mongoose.Schema({
  ...
  file:     { type: String, default: '' }, // MP3 filename
...
// ── Seed Data (only runs if DB is empty) ──
async function seedData() {
  const count = await Song.countDocuments();
  if (count > 0) {
    console.log(`ℹ️  DB already has ${count} songs, skipping seed.`);
    return;
  }
  ...
  console.log('✅ 14 songs seeded into MongoDB');
}
...
app.listen(PORT, () => {
  console.log(`\n🎵 BEET FLOW backend running at http://localhost:${PORT}`);
  console.log(`📡 API endpoints:`);
  console.log(`   GET  http://localhost:${PORT}/api/health`);
  console.log(`   GET  http://localhost:${PORT}/api/songs`);
  console.log(`   POST http://localhost:${PORT}/api/songs/:id/like`);
  console.log(`   POST http://localhost:${PORT}/api/songs/:id/play\n`);
});